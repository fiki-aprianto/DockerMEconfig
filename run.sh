...
for i in `seq 1 ${#IP_ADDRESSES[@]}`; do
  let address_index=$i-1
   ...
   CONTAINER_ID=`docker start fiki`
   BASH_PID=`docker inspect -f {{.State.Pid}} ${CONTAINER_ID}`
  ...
  echo "export mac_veth${i}2=${hwaddr}" >> ./dataconfig.env
  echo "export ip_veth${i}2=${ipaddr}" >> ./dataconfig.env
  echo "export netmask_veth${i}2=${netmask}" >> ./dataconfig.env
done
